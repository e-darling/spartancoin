...
def main() -> None:
    ...
    a, d, df2 = [d for d in datasets if d[0] == 256 and d[1] == 15][0]
    # largest 512 + difficulty 15
    a, d, df5 = [d for d in datasets if d[0] == 512 and d[1] == 15][0]

    p = 1 / 2 ** 15
    x = np.arange(1, df2.tries.max())
    theo = p * (1 - p) ** (x - 1)
    ...
    tries2 = list(
        zip(*sorted([(d, df.tries.mean()) for a, d, df in datasets if a == 256]))
    )
    tries5 = list(
        zip(*sorted([(d, df.tries.mean()) for a, d, df in datasets if a == 512]))
    )
